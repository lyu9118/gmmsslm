...
#' @param parlist A list containing the required parameters (p, g, pi, mu, and sigma) from sslparlist() or provided manually.
...
#' parlist<-sslparlist(fit_pc)
#' erate(dat=dat$Y,parlist=parlist,clust=dat$clust)
#' }
#'
erate<-function(dat,parlist,clust){
  n<-dim(dat)[1]
  g<-parlist$g
  est_clust<-bayesclassifier(dat,parlist)
  prob<-sapply(1:g,function(j)(1-sum(est_clust[clust==j]==j)/sum(clust==j)))
  errval<-sum(parlist$pi*(prob))
  return(errval)
}
